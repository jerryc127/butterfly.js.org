...
很多人使用[hexo-douban](https://github.com/mythsman/hexo-douban)這款插件。但是該插件是直接生成相應的網頁，而不會去讀取對應的markdown文件。因此，可在設置裏進行相應的配置。
...
由於豆瓣開啟了反盜鏈，因此，一些瀏覽器（safari）會無法顯示圖片，可把`meta`設為true。**但此操作會導致一些插件失效（例如：不蒜子）。**
...
百度會將網頁自行轉碼，導致頁面錯亂，甚至插入廣告。開啟後，將會禁止百度轉碼。
